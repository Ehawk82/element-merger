...
function openFullscreen(x, y) {
  return function () {
    (x.disabled = true), (y.disabled = false);
    if (document.body.requestFullscreen) {
      return document.body.requestFullscreen();
    } else if (document.body.mozRequestFullScreen) {
      /* Firefox */ document.body.mozRequestFullScreen();
    } else if (document.body.webkitRequestFullscreen) {
      /* Chrome, Safari & Opera */ document.body.webkitRequestFullscreen();
    } else if (document.body.msRequestFullscreen) {
      /* IE/Edge */ document.body.msRequestFullscreen();
    }
  };
}
function closeFullScreen(x, y) {
  return function () {
    (x.disabled = true), (y.disabled = false);
    if (document.exitFullscreen) {
      ...
      /* Chrome, Safari and Opera */ document.webkitExitFullscreen();
...
function inputValidate(x, y) {
  return function () {
    var d = x.value.trim();
    if (d.length === 0) {
      return false;
    } else {
      x.value = ""; /*work in progress*/
    }
  };
}
...
/* session storage */
function saveSS(x, y) {
  return sessionStorage.setItem(x, JSON.stringify(y));
}
function loadSS(x) {
...
function removeSSitem(x) {
  return sessionStorage.removeItem(x);
}
function SSinit(x, y) {
  var keyname = sessionStorage.getItem(x);
  if (!keyname || keyname === null) {
